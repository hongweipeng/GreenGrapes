...
$comments = $this->comments()->on(true);
?>

<div id="comments">
    <?php if ($comments->have()): ?>
        <div class="comments-block">
            <p class="ui <?php $this->options->singleColor() ?> ribbon badge comments">
                <?php $this->commentsNum(_t('还不快抢沙发'), _t('只有地板了'), _t('<span class="comment-highlight">%d</span> 条评论')); ?>
            </p>
    ...
    <?php if(!$this->allow('comment')): ?>
        <div class="comments-block">
            <p class="ui ribbon badge <?php $this->options->singleColor() ?>"><?php _e('作者关闭了评论'); ?></p>
        </div>
    <?php else: ?>
        <div class="comments-block new-comment" id="<?php $this->respondId(); ?>">
            ...
            <form method="post" action="<?php $this->commentUrl() ?>" class="ui fluid form">
                ...
                <div class="comments-field">
                    <label for="comment-content"></label><textarea rows="8" cols="50" id="comment-content" placeholder="<?php _e('回复内容'); ?><?php _e(' (必填)')?>" name="text"><?php $this->remember('text'); ?></textarea>
                </div>
