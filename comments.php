...
$te_connect_active = class_exists('TeConnect_Plugin') && isset($this->options->plugins['activated']['TeConnect']);
?>
...
<div id="comments">
    <div class="comments-block new-comment" id="<?php $this->respondId(); ?>">
        ...
        <form method="post" action="<?php $this->commentUrl() ?>" class="ui fluid form">
            <?php if($te_connect_active ? TeConnect_Plugin::hasConnectLogin() : $this->user->hasLogin()): ?>
                <div class="comments-field"><?php _e('登录身份：'); ?><a href="javascript:void(0);"><?php $te_connect_active ? _e(TeConnect_Plugin::nickname()) :  $this->remember('author'); ?></a>. <a href="<?php $te_connect_active ? _e(TeConnect_Plugin::logout_url()) : $this->options->logoutUrl(); ?>" title="Logout"><?php _e('退出'); ?> &raquo;</a></div>
                <div class="comments-field">
                    <input type="hidden" name="author" value="<?php $te_connect_active ? _e(TeConnect_Plugin::nickname()) :  $this->remember('author'); ?>" />
            ...
            <?php elseif ($te_connect_active): ?>
                <?php TeConnect_Plugin::show('<p><a href="{url}" class="btn btn-sm btn-outline-success"><i class="fa fa-fw fa-{type}"></i> 登录后评论</a> </p>'); ?>
            <?php else: ?>
