...
class RecentSplitPage extends Widget_Contents_Post_Recent {
...
$page_size = 20;
$recent = Typecho_Widget::widget('RecentSplitPage', ['pageSize' => $page_size, 'currentPage' => $page]);
                        ...
                        <?php while ($recent->next()): ?>
                            <?php if ($year !== date('Y', $recent->created)) :?>
                                ...
                                <div class="timeline-big-dot">
                                    <span class="timeline-year mt-4 mb-4 ml-3"><?php _e($year); ?></span>
                                </div>
                            <?php endif; ?>
                            <div class="line">
                                <div class="d-flex line-item">
                                    <div class="small mr-2">
                                        <time datetime="<?php $recent->date("Y-m-d H:i:s"); ?>" content="<?php $recent->date('Y-m-d'); ?>" data-toggle="tooltip" title="<?php $recent->date("Y-m-d H:i:s"); ?>">
                                            <?php $recent->date('m-d'); ?>
                                        </time>
                                    </div>
                                    <div class="d-inline flex-grow-1">
                                        <a href="<?php $recent->permalink() ?>" data-trigger="hover" data-content="<?php $recent->excerpt() ?>" data-toggle="popover" itemprop="url" target="_blank" rel="noopener noreferrer">
                                            <?php $recent->title() ?>
                                        </a>
                                    </div>
                                </div>
